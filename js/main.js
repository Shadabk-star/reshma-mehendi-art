...
jQuery(document).on('ready', function() {
	...
	function syncPosition(el) {
			if (!sync2.length || !sync2.data('owl.carousel')) {
				return;
			}
	...
	sync2
		...
		.removeClass("current")
		.filter(function() {
			return sync2.data('owl.carousel').relative(jQuery(this).index()) === current;
		})
		.addClass("current");
	...
	if(syncedSecondary) {
		var number = sync2.data('owl.carousel').relative(el.item.index);
		sync1.data('owl.carousel').to(number, 100, true);
		}
	}
	sync2.on("click", ".owl-item", function(e){
		e.preventDefault();
		var number = sync2.data('owl.carousel').relative(jQuery(this).index());
	...
	_hb_counters.appear(function () {
		var _hb_counter = jQuery('.hb-counter > h3 > span[data-to]');
		_hb_counter.countTo();
	});
	/* ---------------------------------------
